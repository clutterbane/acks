...
from proficiency_helpers import (
    add_proficiency,
    apply_auto_proficiencies,
    apply_level_proficiencies,
    choose_class_proficiency,
    general_proficiency_bonus,
)
...
def run_generator(final_class, level, mode="heroic", style="ancient_myth"):
    ...
    # PHYSICAL FEATURES
    physical_feats = generate_physical_features(stats["CHA"])
    ...
    # 1. Auto proficiencies
    auto_general_log = apply_auto_proficiencies(
        final_class, level, char_profs,
        proficiency_limits,
        class_starting_general_proficiencies
    )

    # 2. Class proficiency (1 at level 1)
    class_prof = choose_class_proficiency(final_class, char_profs, proficiency_limits, class_proficiency_lists)

    # 3. General proficiencies (1 + INT bonus)
    general_bonus = general_proficiency_bonus(stats["INT"])
    general_slots = 1 + general_bonus
    general_picks = []
    ...
    # Adventuring always auto
    add_proficiency("Adventuring", char_profs, proficiency_limits)
    general_picks.append("Adventuring")

    # 4. Level-up proficiencies (2+)
    level_gains_log = apply_level_proficiencies(
        final_class, level, char_profs,
        proficiency_limits,
    ...
    # AUTO PROFICIENCIES
    for lvl, kind, prof in auto_general_log:
        p(f"Level {lvl} ({kind}): {prof}")

    # CLASS PROFICIENCY
    if class_prof:
        p(f"Level 1 (class): {class_prof}")

    # GENERAL PROFICIENCIES
    for prof in general_picks:
        p(f"Level 1 (general): {prof}")

    # LEVEL GAINS
    for lvl, kind, prof in level_gains_log:
        p(f"Level {lvl} ({kind}): {prof}")
