...
# pełna lista możliwych typów broni i pancerzy z magic_items.py
KNOWN_WEAPON_SUBTYPES = [
    "Dagger", "Short Sword", "Sword", "Two-Handed Sword",
    "Battle Axe", "Hand Axe", "Great Axe",
    "Bow, Long", "Bow, Short", "Bow, Composite", "Crossbow", "Arbalest",
    "Javelin", "Lance", "Pole Arm", "Spear",
    "Club", "Mace", "Flail", "Morning Star", "Warhammer",
    "Net", "Whip", "Sling", "Cestus", "Staff", "Sap", "Staff-Sling", "Bola",
]

KNOWN_ARMOR_SUBTYPES = [
    "Armor",
    ...
    "Banded Plate Armor",
    "Lamellar Armor",
    "Lamellar Armor, Light",
    "Lamellar Armor, Heavy",
...
def _infer_subtype_from_name(name: str) -> str | None:
    """
    Extract subtype for weapons/armor based on magic_items.py exact naming.
    Fully supports composite names, arena armor variants, laminated armor,
    two-word / three-word types, etc.
    """

    n = name.strip().rstrip("^").strip()

    # Remove suffix such as ", Blazing", ", +2 versus X"
    base = n.split(",")[0].strip()

    # Remove trailing "+X"
    base = re.sub(r"\+\d+.*$", "", base).strip()

    # 1) Try full match in armor list
    for st in KNOWN_ARMOR_SUBTYPES + KNOWN_WEAPON_SUBTYPES:
        if base.startswith(st):
            return st

    # 2) Try substring match (for cases like "Arena Armor, Light")
    for st in KNOWN_ARMOR_SUBTYPES + KNOWN_WEAPON_SUBTYPES:
        if st in base:
            return st

    # 3) Generic weapon / generic armor
    if base.lower().startswith("weapon"):
        return "Weapon"
    if base.lower().startswith("armor"):
        return "Armor"

    # 4) Fallback: first word (rarely used)
    return base.split()[0]

def _finalize_item_subtype(item: dict, style: str) -> dict:
    """
    Forces generic Weapon / Sword / Armor to become specific subtypes.
    """
    category = item["category"]
    name = item["name"]
    subtype = item["subtype"]

    # Only modify generic categories
    if subtype not in ("Weapon", "Sword", "Armor"):
        return item

    # Choose correct specific subtype
    if subtype == "Weapon":
        specific = _choose_misc_weapon_subtype(style)
    elif subtype == "Sword":
        specific = _choose_sword_subtype(style)
    else:  # Armor
        specific = _choose_armor_subtype(style)

    # --- Extract +X bonus robustly ---
    bonus_match = re.search(r"\+\d+", name)
    bonus = bonus_match.group(0) if bonus_match else ""

    # Extract anything after the bonus as suffix (e.g., ", Flaming")
    suffix = ""
    if bonus_match:
        suffix = name[bonus_match.end():].strip(" ,")

    # --- Build new proper name ---
    new_name = specific
    if bonus:
        new_name += f" {bonus}"
    if suffix:
...
def generate_magic_by_category(category: str, campaign_style: str):
    """
    API używane przez classic treasure generator.
    category może być:
    - "any"
    - "potion"
    - "scroll"
    ...
    # --- ANY → pełna klasyczna tabela magic items ---
    if category == "any":
        return generate_classic_item(campaign_style, "random")

    # --- POTION ---
    if category == "potion":
        return _classic_potions(campaign_style)

    # --- SCROLL ---
    if category == "scroll":
        return _classic_scrolls(campaign_style)

    # --- RING ---
    if category == "ring":
        return _classic_rings(campaign_style)

    # --- WEAPON / ARMOR ---
    if category == "weapon_armor":
        # sword vs misc weapon vs armor (1/3 każdy)
        r = random.randint(1, 3)
        if r == 1:
            return _classic_swords(campaign_style)
        elif r == 2:
            return _classic_misc_weapons(campaign_style)
        else:
            return _classic_armors(campaign_style)

    # --- Specific: weapon ---
    if category == "weapon":
        # sword lub misc-weapon
        return random.choice([
            _classic_swords(campaign_style),
            _classic_misc_weapons(campaign_style)
        ])
    ...
    if category == "misc_weapon":
        return _classic_misc_weapons(campaign_style)

    if category == "misc_item":
        return _classic_misc_items(campaign_style)

    if category == "implement":
        return _classic_implements(campaign_style)

    raise ValueError(f"Unknown magic item category: {category}")
